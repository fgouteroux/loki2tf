...
// Adapted to convert loki prometheus yaml rules to HashiCorp's Terraform configuration language.
...
func init() {
	...
	flag.BoolVarP(&printVersion, "version", "v", false, `Print loki2tf version`)
...
func main() {
	...
	if reverse {
		...
		for _, obj := range objs {
			for key, value := range obj["resource"].(map[string]interface{}) {
				if key == "loki_rule_group_alerting" {
				...
				if key == "loki_rule_group_recording" {
	...
	// generate loki_rule_group_alerting terraform resources
	for _, obj := range objs {
		for _, group := range obj.Groups {
			var alertingRule bool
			var resource string
			resource = fmt.Sprintf("resource \"loki_rule_group_alerting\" \"%s\" {\n", group.Name)
	...
	// generate loki_rule_group_recording terraform resources
	for _, obj := range objs {
		for _, group := range obj.Groups {
			var recordingRule bool
			var resource string
			resource = fmt.Sprintf("resource \"loki_rule_group_recording\" \"%s\" {\n", group.Name)
			resource += fmt.Sprintf("name = \"%s\"\n", group.Name)
			if obj.Namespace != "" {
				resource += fmt.Sprintf("namespace = \"%s\"\n", obj.Namespace)
			}
